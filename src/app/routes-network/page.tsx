...
import { MapPin, Plane, Clock, Filter } from 'lucide-react'
import Link from 'next/link'
import { AIRPORTS, formatDuration } from '@/lib/utils'
...
export default function RoutesPage() {
  ...
  const filtered = ROUTES.filter(r => {
    const matchesSearch = search === '' ||
      AIRPORTS[r.from]?.city.toLowerCase().includes(search.toLowerCase()) ||
      AIRPORTS[r.to]?.city.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="min-h-screen pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <p className="text-xs tracking-widest uppercase font-semibold mb-3" style={{ color: 'var(--indigo-accent)' }}>Route Network</p>
        ...
        >
          <div className="absolute inset-0 opacity-5" style={{
            backgroundImage: 'radial-gradient(circle at 50% 50%, var(--indigo-primary) 1px, transparent 1px)',
            backgroundSize: '40px 40px'
          }} />
          <div className="relative">
            <div className="text-6xl mb-3">🌍</div>
            <h3 className="font-display font-bold text-2xl mb-2" style={{ color: 'var(--text-primary)' }}>Global Network</h3>
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Connecting {Object.keys(AIRPORTS).length} cities across the world</p>
            <div className="flex flex-wrap justify-center gap-3 mt-6">
              {Object.keys(AIRPORTS).slice(0, 12).map(code => (
                <span key={code} className="px-3 py-1 rounded-full text-xs font-mono font-bold"
                  style={{ background: 'rgba(99,102,241,0.1)', color: 'var(--indigo-accent)' }}>
                  {code}
                </span>
              ))}
              <span className="px-3 py-1 rounded-full text-xs" style={{ color: 'var(--text-muted)' }}>+{Object.keys(AIRPORTS).length - 12} more</span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            <MapPin size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'var(--indigo-accent)' }} />
            <input
              type="text"
        ...
        <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>{filtered.length} routes shown</p>
                  ...
                  >
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-mono font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{route.from}</div>
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{AIRPORTS[route.from]?.city}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-mono font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{route.to}</div>
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{AIRPORTS[route.to]?.city}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{route.frequency}</span>
                    </td>
                    <td>
